...
/** Current OTP/reset challenge held only in browser memory. */
let srccfpcAuthChallenge = "";
let srccfpcResetToken = "";

/** Handles OTP verification for login or password reset. */
async function handleOtpSubmit(event) {
  event.preventDefault();
  try {
    const result = await SRCCFPC.callAPI("verifyOtp", { challenge: srccfpcAuthChallenge, otp: document.getElementById("otpInput").value }, null);
    if (result.token) {
      localStorage.setItem("srccfpc_session", result.token);
      localStorage.setItem("srccfpc_user", JSON.stringify(result.user));
      const next = new URLSearchParams(window.location.search).get("next");
      window.location.assign(next && /^[A-Za-z0-9_-]+\.html$/.test(next) ? next : "member-detail.html");
    } else if (result.resetToken) {
      srccfpcResetToken = result.resetToken;
...
async function handleForgotPassword(event) {
  event.preventDefault();
  try {
    const result = await SRCCFPC.callAPI("forgotPassword", { username: document.getElementById("forgotUsername").value }, null);
    srccfpcAuthChallenge = result.challenge;
...
async function handleResetPassword(event) {
  event.preventDefault();
  try {
    const result = await SRCCFPC.callAPI("resetPassword", { resetToken: srccfpcResetToken, newPassword: document.getElementById("resetPassword").value }, null);
    SRCCFPC.showToast(result.message);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  loginForm?.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      const result = await SRCCFPC.callAPI("login", data, null);
      if (result.otpRequired) {
        srccfpcAuthChallenge = result.challenge;
        document.getElementById("otpPanel").hidden = false;
        document.getElementById("forgotPanel").hidden = true;
        loginForm.closest(".glass-card")?.classList.add("otp-pending");
        SRCCFPC.showToast(result.message);
      }
    } catch (error) { SRCCFPC.showToast(error.message); }
  ...
  document.getElementById("forgotPasswordButton")?.addEventListener("click", () => { document.getElementById("forgotPanel").hidden = false; });
});
