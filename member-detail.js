...
// This MUST be the exact same Web App URL used in every page's JS file
// (admin.js, join.js, login.js, and every other page listed here).
...
/**
 * Generic page-specific controller.
 */

/**
 * Performs page initialization that does not belong
...
/** Loads the authenticated member profile into the portal. */
async function loadMemberPortalProfile() {
  try {
    const result = await SRCCFPC.callAPI("profile", {});
    const user = result.profile || result.user || {};
    document.getElementById("memberName").textContent = user.full_name || user.name || "Member";
    document.getElementById("memberMeta").textContent = user.department_stream || "SRCCFPC Member";
    document.getElementById("memberRoll").textContent = user.roll_number || "—";
    document.getElementById("memberUsername").textContent = user.username || "—";
...
/** Loads persistent member chat history from the backend. */
async function loadMemberChat() {
  const box = document.getElementById("chatMessages");
  if (!box) return;
  try {
    const result = await SRCCFPC.callAPI("getChatMessages", { limit: 80 });
    const messages = result.messages || [];
    if (!messages.length) { box.innerHTML = '<div class="chat-empty">No messages yet. Start the conversation.</div>'; return; }
    box.innerHTML = messages.map((item) => {
      const own = item.sender_roll_number === (JSON.parse(localStorage.getItem("srccfpc_user") || "{}").roll_number || "");
      const bubble = document.createElement("div");
      bubble.className = `chat-message ${own ? "chat-message--own" : ""}`;
      const sender = document.createElement("strong"); sender.textContent = item.sender_name || item.sender_username || "Member";
  ...
  } catch (error) { box.innerHTML = `<div class="chat-empty">${error.message}</div>`; }
}

/** Handles persistent chat submission. */
async function submitMemberChat(event) {
  event.preventDefault();
  const input = document.getElementById("chatMessage");
  const button = document.getElementById("sendChat");
  button.disabled = true;
  try {
    await SRCCFPC.callAPI("sendChatMessage", { message: input.value });
    input.value = "";
    await loadMemberChat();
  } catch (error) { SRCCFPC.showToast(error.message); }
  finally { button.disabled = false; }
}

/** Handles the fourteen-day username change workflow. */
async function submitUsernameChange(event) {
  event.preventDefault();
  try {
    const result = await SRCCFPC.callAPI("changeUsername", { currentPassword: document.getElementById("usernameCurrentPassword").value, newUsername: document.getElementById("newUsername").value });
    SRCCFPC.showToast(result.message);
    localStorage.removeItem("srccfpc_session");
    localStorage.removeItem("srccfpc_user");
...
async function submitPasswordChange(event) {
  ...
  if (newPassword !== confirmPassword) return SRCCFPC.showToast("New password and confirmation do not match.");
  if (!confirm("Are you sure you want to change your password?")) return;
  try { const result = await SRCCFPC.callAPI("changePassword", { currentPassword, newPassword }); SRCCFPC.showToast(result.message); event.currentTarget.reset(); } catch (error) { SRCCFPC.showToast(error.message); }
}

document.addEventListener("DOMContentLoaded", () => {
  loadMemberPortalProfile();
  loadMemberChat();
  document.getElementById("chatForm")?.addEventListener("submit", submitMemberChat);
  document.getElementById("refreshChat")?.addEventListener("click", loadMemberChat);
  document.getElementById("usernameForm")?.addEventListener("submit", submitUsernameChange);
  document.getElementById("passwordForm")?.addEventListener("submit", submitPasswordChange);
  document.querySelectorAll("[data-toggle-password]").forEach((button) => button.addEventListener("click", () => { const input = document.getElementById(button.dataset.togglePassword); const show = input.type === "password"; input.type = show ? "text" : "password"; button.textContent = show ? "Hide" : "Show"; }));
